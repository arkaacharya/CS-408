...
?>
	<html>
	<head>
	<title>Add Chapter</title> <!-- Name on the tab of the webpage -->
	...
	<font size="+2" face="arial"><center><header><h1>Add Chapter for Existing User</h1></header></center></font>
	...
	<tr>
		...
		<?php
			...
			$result = mysqli_fetch_row($data1); //Extracting information fro mthe executed query
			
			$userName = $_GET['userName']; //Getting the username
			
			while($result){ //Condition for looping as long as there is information passed in the query
				$check = true; //Initializing a check variable
				$sql = "SELECT chapter FROM ".$userName; //Constructing an sql query to get the name of a chapter which the user is allowed to access
				$data = mysqli_query($conn, $sql); //Executing the sql query
				$resultNew = mysqli_fetch_row($data); //Extracting information from the executed query
				while($resultNew){ //Condition for looping as long as there is information passed in the query
					if($resultNew[0] == $result[0]) //Checking if the user already has access to the chapter
						$check = false; //Changing the value of the check variable
					$resultNew = mysqli_fetch_row($data); //Extracting information from the executed query
				}
				if($check){ //Checking the value of the check variable
			?>
	...
	<tr>
		...
		<font size="+2" face="arial"><a href="addChap.php?acessor=admin">Back</a></font>
		</br></br>
	<font size="+2" face="arial"><a href="adminPage.php?acessor=admin">Home</a></font size="+2">
		...
		<?php
			if(isset($_POST['chapter'])){ //Checking if the chapter has been set
				...
				$flag = 1; //Used as a counter
				$sql = "SELECT chapter FROM ".$userName; //Constructing an sql query to check how many chapters the user can access
				$data = mysqli_query($conn, $sql); //Executing the sql query
				$result = mysqli_fetch_row($data); //Extracting infromation from executed query
				while($result){ //Condition for looping as long as there is information passed in the query
					$flag++; //Incrementing the counter
					$result = mysqli_fetch_row($data); //Extracting information from the executed query
				}
				
				$sql = "INSERT INTO ".$userName." (flag, chapter) VALUES (".$flag.", \"".$chapter."\")"; //Constructing an sql query to insert the new chapter into the user's table
				$data = mysqli_query($conn, $sql); //Executing the sql query
				
				header("Location: addChap.php?acessor=admin"); //Redirecting to the admin page
