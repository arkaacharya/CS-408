...
?>
	...
	<tr>
		...
		<?php
			...
			$sql = "SELECT formNo, username, essayGraded FROM useranswers";
			...
			while($result){ //Condition to loop as long as information is being received
				if($result[1] == $userName){ //Checking if the username received is a match
	...
	<tr>
		...
		<font size="+2" face="arial"><a href="selectUser.php?acessor=admin&redirect=<?php echo $redirect; ?>">Back</a></font size="+2">
		</br></br>
		...
		<?php
			if(isset($_POST['testName'])){ //Checking if a test has been selected
				...
				header("Location: ".$redirect.".php?acessor=admin&userName=".$userName."&testName=".$testName); //Redirecting to the appropriate page
