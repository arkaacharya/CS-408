...
?>
	...
	<table border = "0">
	<!-- Displaying the test names in a group of radio buttons -->
		<tr><td>
	...
	<font size="+2" face="arial"><a href="adminPage.php?acessor=admin">Home</a></font size="+2">
		...
		<?php
			if(isset($_POST['testName'])){ //Checking if a test has been selected
				$testName = $_POST['testName']; //Getting the name of the slected test

				//Constructing an sql query to check if the test exists
				$sql = "SELECT flag FROM form WHERE formNo = \"".$testName."\"";
				$data = mysqli_query($conn, $sql); //Executing query
				$result = mysqli_fetch_row($data); //Extracting information 
				
				if(!$result){ //Checking if information was returned
					header("Location: testDoesntExist.phpacessor=admin"); //Redirecting to the error page
